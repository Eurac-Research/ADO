...
export default function RegionDetail({
  ...
  const [staticMetaData, setStaticMetaData] = useState<any>(
    initialStaticMetaData || null
  )
  const [isLoadingMeta, setIsLoadingMeta] = useState(
    !initialStaticMetaData || !initialNutsName
  )
  ...
  const [selectedComparisonRegion, setSelectedComparisonRegion] = useState<
    string | null
  >(null)
  const [comparisonRegionData, setComparisonRegionData] = useState<
    TimeSeriesData[] | null
  >(null)
    ...
    const csvContent = [
      ...
      ...nutsData.map((row) => {
        return [
          row.date,
          ...indices.map((idx) =>
            row[idx.toUpperCase()] !== undefined &&
            row[idx.toUpperCase()] !== null
              ? row[idx.toUpperCase()]
              : ''
          ),
        ].join(',')
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!isTimeSeriesArray(data)) {
          console.error(
            'Unexpected time series payload for region:',
            nutsId,
            data
          )
          setNutsData(null)
  ...
  if (metaError) {
    ...
    if (mode === 'page') {
      return (
        <Layout posts={allPosts} headerMode={1}>
          <div className="min-h-screen bg-gray-50 dark:bg-gray-900 mt-20">
            <div className="container mx-auto px-4 py-8">{errorContent}</div>
          </div>
        </Layout>
      )
    }
  ...
  const mainContent = (
    <div
      ...
      data-name="region-detail"
    >
      {mode === 'modal' && (
        <div
          className="overlayContainer fixed inset-0 bg-black bg-opacity-50"
          onClick={onClose}
        />
      )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <button
                  ...
                  title={
                    compareRegions
                      ? 'Disabled: Region comparison is active'
                      : 'Compare data across different years'
                  }
                >
                  Compare by Year
                </button>
                ...
                <button
                  ...
                  title={
                    compareYears
                      ? 'Disabled: Year comparison is active'
              ...
              <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                ...
                <div className="space-y-3">
                  <select
                    ...
                    onChange={(e) =>
                      setSelectedComparisonRegion(e.target.value || null)
                    }
                  ...
                  {selectedComparisonRegion &&
                    ...
                    !comparisonRegionData && (
                      <p className="text-sm text-red-500 dark:text-red-400">
                        Failed to load data for selected region.
                      </p>
                    )}
                    ...
                    <p className="text-sm text-green-600 dark:text-green-400">
                      ✓ Loaded data for{' '}
                      {
                        availableRegions.find(
                          (r) => r.id === selectedComparisonRegion
                        )?.name
                      }
                    </p>
                  )}
        ...
        {!isLoading && !isError && nutsData && staticMetaData && (
          <div className="mb-8 bg-gray-50 dark:bg-gray-700 rounded-lg p-2 sm:p-4">
            <TimeSeries
              ...
              firstDate={format(
                new Date(
                  new Date(day).getTime() - 5 * 365 * 24 * 60 * 60 * 1000
                ),
                'YYYY-MM-DD'
              )}
              lastDate={day}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
            ...
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <dd className="font-mono text-blue-900 dark:text-blue-100">
                  {nutsId}
                </dd>
              </div>
              <div className="flex justify-between">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            ...
            <div className="space-y-3">
              ...
              {nutsData &&
                (staticMetaData?.factsheet || staticMetaData?.doi) && (
